...
//            fos.close();
	...
	@ResponseBody
	//@PreAuthorize("hasAuthority('fdfs:download')")
	public CommonResult download(@RequestParam("fdfsGroup") String fdfsGroup,
								 @RequestParam("fdfsFullPathName") String fdfsFullPathName) {
		FdfsFileDto fdfsFileDto = new FdfsFileDto();
		byte[]  fileContext= fastFileStorageClient.downloadFile(fdfsGroup, fdfsFullPathName);
		fdfsFileDto.setFdfsFullName(fdfsFullPathName);
		fdfsFileDto.setFdfsGroup(fdfsGroup);
